...
<<<<<<< HEAD
Furthermore, there have been several high-profile password thefts in recent years where servers stored passwords insecurely (in particular, in plain text or hashed without salting) and were subsequently compromised.  Such thefts are the most publicized form of large-scale password leakage.  When a user sends a password in plaintext and the server does not store this password securely, such catastrophes may occur.  For instance, in January 2013, the servers of dating network Cupid Media were compromised, resulting in the theft of a staggering 42 million unencrypted passwords.  \cite{Donohue}
=======
...
>>>>>>> 2bea54e356f94a0eb3f8d03539220d320486a4d3
...
To authenticate, the server first generates, stores, and sends the user a challenge in the form of a random number $a$, tying it to the user's username.  The user then inputs a password, and the client calculates the following values:
\begin{itemize}
...
For a correctly entered password, $T' = Y^cg_0z = g_0^{xc + r - cx} = g_0^r =
T$, and the server's value of $Y$ will match the value computed by the user.  Thus, the two values of $c$ will also match and the server will always authenticate correctly if given valid credentials.  Refer to Lum Jia Jun's original paper for a more in-depth discussion of correctness and security.
